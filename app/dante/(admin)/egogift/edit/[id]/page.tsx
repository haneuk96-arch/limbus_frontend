...
export default function EgoGiftEditPage() {
  ...
  const [priorityYn, setPriorityYn] = useState("N");
  ...
  const fetchEgoGift = async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const egogift = data.egogift;
        setKeywordId(egogift.keywordId !== null && egogift.keywordId !== undefined ? String(egogift.keywordId) : "0");
        setAttrKeywordId(egogift.attrKeywordId !== null && egogift.attrKeywordId !== undefined ? String(egogift.attrKeywordId) : "");
        setGiftName(egogift.giftName || "");
        setGiftTier(egogift.giftTier || "");
        setCost(egogift.cost ? String(egogift.cost) : "");
        setEnhanceYn(egogift.enhanceYn || "Y");
        setSynthesisYn(egogift.synthesisYn || "N");
        const py = (egogift as { priorityYn?: string; priority_yn?: string }).priorityYn ?? (egogift as { priority_yn?: string }).priority_yn;
        setPriorityYn(py === "Y" ? "Y" : "N");
        ...
        setDesc3(egogift.desc3 || "");
        setSelectedTagIds(egogift.tagIds || []);
      ...
      setError("에고기프트 정보를 불러오는데 실패했습니다.");
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // 수정한 키워드 ID를 쿼리 파라미터로 전달
        router.push(`/dante/egogift?keywordId=${finalKeywordId}`);
      } else {
        setError("수정에 실패했습니다.");
      }
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-yellow-300">에고기프트 수정</h1>
        <Link
          href="/dante/egogift"
