...
export default function EgoGiftRecipeCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_URL}/admin/egogift/recipe/create`, {
        method: "POST",
        headers: {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-yellow-300">에고기프트 조합식 등록</h1>
        <Link
