...
export default function DungeonEventEditPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const loadedNodes = (data.nodes || []).map((n: any) => ({
          ...
          eventType: n.eventType ?? 0,
          egogiftIds: (n.egogiftIds || []).map((id: any) => Number(id)),
        }));
        setNodes(loadedNodes);
        
        // 모든 노드를 닫힌 상태로 설정
        setCollapsedNodeContents(new Set(loadedNodes.map((n: any) => n.clientKey)));
        ...
        if (data.attachment) {
          ...
          setPreview(`${baseUrl}${data.attachment.path}`);
        }
      } else {
    ...
    try {
      ...
      if (res.ok) {
        ...
        try {
          ...
          setError(resp.message || "수정에 실패했습니다.");
        } catch {
          setError("수정에 실패했습니다.");
        }
      }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-yellow-300">던전 이벤트 수정</h1>
        <Link
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {existingFile && !file && (
                  <div className="space-y-2">
                    <div className="text-gray-400 text-xs">
                      기존 파일: {existingFile.originalName}
                    </div>
                    <button
                      type="button"
                      onClick={handleDeleteFile}
                      className="px-3 py-1 text-xs bg-red-700 hover:bg-red-800 text-white rounded"
                    >
                      파일 삭제
                    </button>
                  </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mb-3">
              부모를 지정해 선택지 간의 상하 구조를 구성할 수 있습니다.
            </p>
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="text-sm text-yellow-300 font-medium flex-1">
                          {(() => {
                            ...
                            const nodeTitle = node.title || (node.nodeId ? `Node ${node.nodeId}` : `Node ${idx + 1}`);
                            if (parentTitle) {
                              return (
                                <>
                      ...
                      <div className="space-y-1">
                        <div className="flex gap-2 items-center mb-1">
                          ...
                          <input
                            type="text"
                            value={nodeReplaceTexts[node.clientKey] || ""}
                            onChange={(e) => {
                              setNodeReplaceTexts(prev => ({
                                ...prev,
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const nodeReplaceText = nodeReplaceTexts[node.clientKey] || "";
                              if (!nodeReplaceText || !node.description) return;
                              const escapedText = nodeReplaceText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
