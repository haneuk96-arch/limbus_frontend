...
export default function CardPackCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/admin/dungeon/cardpack`, {
        method: "POST",
        credentials: "include",
        body: formData,
      });
      ...
      if (res.ok) {
        const cardPackData = await res.json();
        const cardpackId = cardPackData.cardpackId;
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-yellow-300">카드팩 등록</h1>
        <Link
            ...
            {preview && (
              <div className="mt-3">
                <img
                  src={preview}
                  alt="미리보기"
                  className="max-w-xs max-h-48 object-contain border border-red-700/50 rounded"
                />
              </div>
            )}
          </div>
